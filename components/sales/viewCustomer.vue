          ...
          <v-card>
            ...
            <v-card-text class="pa-0">
              <v-container grid-list-xs>
                <csAddComponent v-if="showModal==true" v-model="closeModal1" mode="customer" :id="editValue"></csAddComponent>
              </v-container>
            </v-card-text>
              ...
              <v-container grid-list-xs v-if="showBulk==true">
                <v-layout align-center justify-start row wrap>
                  ...
                  <v-flex xs7 sm3>
                    ...
                    <a :href="url+'/'+$route.params.username+'/api/fileUploads/SampleFiles/download/AddCustomer.xlsx?access_token='+this.$store.state.token" download ref="sampleFile"> </a>
                  </v-flex>
                </v-layout>
                <v-layout class="mb-3" row wrap>
                 <v-flex xs10 sm8 class="font-20"><v-icon style="color:red">info </v-icon>  File should be in this specified Format:-</v-flex>
                 <v-flex xs2 sm4 ></v-flex>    
                </v-layout> 
                <v-layout row wrap class="mb-3">
                                    <v-flex sm12 class="font-14">a) Customer name , b) Mobile No , c) Email , d) Street , e) Area , f) Pincode , g) City , h) State , i) Gstn , j) StateCode</v-flex>
                </v-layout> 
                <v-layout row wrap class="mb-3">
                ...
                <v-layout row wrap>
                                  <v-flex sm12>
                                      <!-- begin panel -->
                                    <div  @drop="_drop" @dragenter="_suppress" @dragover="_suppress" data-sortable-id="form-dropzone-1" style="cursor:pointer" @click="$refs.fileInput.click()">
